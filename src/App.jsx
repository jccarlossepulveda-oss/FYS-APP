...
export default function App({ usuario, rol, onLogout }) {
  ...
  const navItem = (v, icon, label, count) => (
    <div onClick={() => setView(v)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 14px', cursor: 'pointer', borderLeft: view === v ? '2px solid ' + r : '2px solid transparent', background: view === v ? '#fff0f2' : 'transparent', color: view === v ? r : m, fontSize: 12, fontWeight: view === v ? 500 : 400 }}>
    ...
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', fontFamily: 'system-ui,sans-serif', fontSize: 13, color: b, background: '#F7F7F6' }}>
      <div style={{ width: 190, flexShrink: 0, background: w, borderRight: br, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '8px 0', flex: 1, overflowY: 'auto' }}>
          ...
          {navItem('flota', '🚛', 'Flota')}
          {navItem('choferes', '👷', 'Choferes')}
          {navItem('viajes', '📍', 'Viajes', vAct || null)}
          ...
          {navItem('gasolina', '⛽', 'Gasolina')}
          {navItem('facturas', '📄', 'Facturas', fPend.length || null)}
          ...
          {navItem('mantenimiento', '🔧', 'Mantenimiento')}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px' }}>
          {view === 'inicio' && <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,minmax(0,1fr))', gap: 10, marginBottom: 16 }}>
              {[[camiones.filter(c => c.estatus === 'en_ruta').length,'🚛 Activas','de '+camiones.length+' en flota'],[vAct,'📍 En curso',viajes.filter(v=>v.estatus==='programado').length+' programados'],[fPend.length,'📄 Pendientes','$'+fPend.reduce((a,f)=>a+Number(f.monto||0),0).toLocaleString()],[choferes.filter(c=>c.activo).length,'👷 Choferes','de '+choferes.length]].map(([val,lbl,sub]) => (
                <div key={lbl} style={{ background: w, border: br, borderRadius: 8, padding: 12 }}><div style={{ fontSize: 11, color: m, marginBottom: 5 }}>{lbl}</div><div style={{ fontSize: 20, fontWeight: 600 }}>{val}</div><div style={{ fontSize: 10, color: '#9E9E9B', marginTop: 2 }}>{sub}</div></div>
              ))}
            </div>
          ...
          {view === 'flota' && <div style={card}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
              ...
              <tbody>{camiones.map(c => <tr key={c.id}>
                <td style={{ ...td, fontWeight: 600 }}>{c.economico}</td><td style={td}>{c.marca}</td><td style={td}>{c.modelo}</td><td style={td}>{c.anio}</td><td style={td}>{c.placas}</td><td style={td}>{c.capacidad}</td>
                ...
                <td style={td}><button style={btnD} onClick={() => eliminar('camiones', c.id, '¿Eliminar '+c.economico+'?')}>🗑 Eliminar</button></td>
              </tr>)}</tbody>
            </table>
          </div>}
          ...
          {view === 'gasolina' && <>
            ...
            <div style={card}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                <thead><tr><th style={th}>Fecha</th><th style={th}>Unidad</th><th style={th}>Litros</th><th style={th}>Monto</th><th style={th}>Gasolinera</th><th style={th}>Km</th></tr></thead>
                <tbody>{gasolina.map(g => {
                  const cam = camiones.find(c => c.id === g.camion_id)
                  return <tr key={g.id}>
                    <td style={td}>{g.fecha}</td>
                    <td style={{ ...td, fontWeight: 600 }}>{cam?.economico || '—'}</td>
                    <td style={td}>{g.litros} lts</td>
                    <td style={{ ...td, fontWeight: 600 }}>${Number(g.monto||0).toLocaleString()}</td>
                    <td style={td}>{g.gasolinera}</td>
                    <td style={td}>{g.kilometraje ? g.kilometraje.toLocaleString() + ' km' : '—'}</td>
                  </tr>
                })}</tbody>
              </table>
